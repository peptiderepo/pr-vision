<?php
/**
 * Tests for the visibility score formula in PGM_Ai_Visibility_Collector.
 *
...
echo "=== PGM Score Calculation Tests ===\n";

// Instantiate with a null ledger (no DB calls in these tests).
$collector = new PGM_Ai_Visibility_Collector( null );

// ── Test: zero total → score 0 ──────────────────────────────────────────

pgm_test_reset();
pgm_assert_equals( 0.0, $collector->compute_score( 0, 0, 0.0 ), 'score: zero total returns 0.0' );
...
// score = (1.0 + 1.0) / 2 = 1.0
pgm_test_reset();
pgm_assert_equals( 1.0, $collector->compute_score( 10, 10, 10.0 ), 'score: all cited at position 1 → 1.0' );
...
pgm_test_reset();
pgm_assert_equals( 0.0, $collector->compute_score( 0, 10, 0.0 ), 'score: none cited → 0.0' );
...
pgm_assert_equals( 0.375, $score, 'score: half cited at position 2 → 0.375' );
...
pgm_assert_equals( 0.6667, $score, 'score: single probe cited at position 3 → 0.6667' );
...
pgm_test_reset();
pgm_assert( $collector->compute_score( 1, 1, 1.0 ) <= 1.0, 'score: never exceeds 1.0' );
pgm_assert( $collector->compute_score( 0, 10, 0.0 ) >= 0.0, 'score: never below 0.0' );
...
pgm_assert_equals( $expected, $score, 'score: 4-decimal precision maintained' );
